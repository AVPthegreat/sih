  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({
        reply:
          "AI is not fully configured yet. Add GEMINI_API_KEY in your environment to enable real responses. For now, here's a sample reply: " +
          `You asked: "${prompt}". A helpful response would go here.`,
      })
    }

    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`
    
    // Double safety: Wrap user prompt with instructions
